...
with st.sidebar:
    ...
    st.caption("Log a contribution towards one of your goals")

    all_goals_sidebar = db.get_all_goals()
    if not all_goals_sidebar:
        st.caption("No savings goals yet — create one in the 🎯 Savings Goals tab.")
    else:
        goal_name_map = {f"{g[1]} (₹{db.get_goal_saved_total(g[0]):,.0f} / ₹{g[2]:,.0f})": g[0] for g in all_goals_sidebar}
        selected_goal_label = st.selectbox("Goal", list(goal_name_map.keys()), key="sidebar_goal_select")
        selected_goal_id = goal_name_map[selected_goal_label]

        with st.form("sidebar_add_savings_form", clear_on_submit=True):
            sv_amount = st.number_input("Amount (₹)", min_value=0.0, step=100.0, format="%.2f", key="sidebar_sv_amount")
            sv_note = st.text_input("Note (optional)", key="sidebar_sv_note")
            sv_submitted = st.form_submit_button("💰 Add Savings", width='stretch')

            if sv_submitted:
                if sv_amount <= 0:
                    st.warning("Enter an amount greater than 0.")
                else:
                    db.add_contribution(selected_goal_id, sv_amount, str(date.today()), sv_note)
                    st.success(f"Added ₹{sv_amount:,.2f} to '{selected_goal_label.split(' (')[0]}'")
                    st.rerun()
        ...
        for week in weeks:
            ...
            for day in week:
                if day == 0:
                    ...
                    if d > today:
                        # Future day — hasn't happened yet, leave it blank/white
                        z_row.append(None)
                        text_row.append(f"{day}")
                        custom_row.append(False)
                    else:
